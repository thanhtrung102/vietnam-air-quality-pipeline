...
- existing analogous services/functions/resources in the repo's infra modules (e.g. `terraform/`,
  `lambda/`, or the stack's equivalent) before proposing a new one
- current deploy, packaging, and scheduling patterns
- current env and secret wiring patterns

Before adding a new Lambda, scheduled job, container, or host-side service, inspect the existing infra
modules to confirm no current resource already covers the operation. If one does, adding a new
sidecar/function/service is probably the wrong design.
