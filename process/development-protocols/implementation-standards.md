...
- Keep source files roughly under 200 lines when practical (any language); split by responsibility when files become hard to reason about.
...
- Use the repo's established package manager / build tool (check the manifest — `package.json`,
  `pyproject.toml`, `poetry.lock`, `requirements.txt`, `dbt_project.yml`, or `*.tf`); do not assume a
  specific one. For this pipeline: Python (`pip`/`requirements.txt`), Terraform, and dbt.
- Use Context7 for library and API docs or setup guidance.
- Use `gh` for GitHub automation when needed.
- For data/database debugging, follow the current repo stack and context docs; do not assume a specific
  ORM or engine unless the code actually uses it.
